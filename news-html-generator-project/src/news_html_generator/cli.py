...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("input_docx", nargs="+", help="Path to the source .docx file(s) or directory")
...
def main() -> None:
    ...
    input_paths = [Path(p) for p in args.input_docx]
    output_path = Path(args.output_html)

    dispatch_paths = input_paths[0] if len(input_paths) == 1 else input_paths

    if output_path.suffix.lower() != ".html":
        generate_intranet_site(dispatch_paths, output_path, site_title=args.site_title)
        return

    html = generate_html(dispatch_paths, page_title=args.title)
